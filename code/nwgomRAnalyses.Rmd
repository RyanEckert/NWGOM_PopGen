...
sintLineageViolinA = ggplot(data = sintPcangsd, aes(x = cluster, y = depthm)) +
  ...
  annotate(geom = "text", x = sintDepthLetters$x, y = (sintDepthLetters$y - 3), label = sintDepthLetters$grp) +
...
```{r, sint pi}
sintNpgList = list(read_tsv("../data/sint/Si_Deep.thetas.idx.pestPG") %>% mutate(lineage = "Si_Deep"),
               read_tsv("../data/sint/Si_1.thetas.idx.pestPG") %>% mutate(lineage = "Si_1"))
...
sintPiAll$lineage = factor(sintPiAll$lineage, levels = levels(sintPiAll$lineage)[c(2,1)])
...
```{r, sint sym}
sintData = read.csv("../data/nwgomMetaData.csv") %>% dplyr::filter(species == "S. intersepta") %>% dplyr::filter(!sample %in% c("SGM007", "SGM010", "SGM013", "SGM027.2", "SGM027.3", "SGM046.1", "SGM046.3", "SGM063", "SGM152.2", "SGM152.3", "SGM169", "SGM177", "SGM179", "SGM186.1", "SGM186.3", "SGM197.2", "SGM197.3", "SGM206.2", "SGM206.3")) %>% dplyr::select("site" = bank, "depth" = depthZone)
